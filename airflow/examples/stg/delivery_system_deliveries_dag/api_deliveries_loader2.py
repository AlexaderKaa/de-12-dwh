...
class DeliveryObj(BaseModel):
    d_order_id: str
    d_order_ts: str  # Убедитесь, что это поле нужно только один раз
    d_delivery_id: str
    d_courier_id: str
    d_address: str
    d_delivery_ts: str
...
class DeliveryOriginRepository:
    def list_deliveries(self) -> List[DeliveryObj]:
        ...
        # Преобразуем каждый словарь в объект DeliveryObj
        objs = [
            DeliveryObj(
                ...
                d_sum=d['sum'],
                d_tip_sum=d['tip_sum']
            ) for d in data if 'order_id' in d  # Проверка на наличие ключа для избежания ошибок
        ]
    ...
    def load_deliveries(self):
        with self.pg_dest.connection() as conn:
            ...
            while True:
                ...
                # Обновляем последний загруженный ID (предполагается что это уникальный идентификатор)
                wf_setting.workflow_settings[self.LAST_LOADED_ID_KEY] = max([t.d_delivery_id for t in load_queue])

                # Сохраняем обновленные настройки в базе данных
