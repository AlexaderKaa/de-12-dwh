...
class DeliveryOriginRepository:
    def list_deliveries(self) -> List[DeliveryObj]:
        ...
        offset = 0
        while True:
            data = deliveries_data.fetch_data(offset=offset)  # Предполагается, что это возвращает список словарей
            ...
            if objs is None or not data:
                print(f'Not data for load')
                break
            offset += deliveries_data.limit
            print(objs)
        return objs
