...
class DeliveryRestaurantOriginRepository:
    def list_delivery_restaurants(self) -> List[DeliveryRestaurantObj]:
        ...
        offset = 0
        while True:
           data = restaurants_data.fetch_data(offset=offset)  # Предполагается, что это возвращает список словарей
           data_objs = [DeliveryRestaurantObj(r_id=restaurant['_id'], r_name=restaurant['name']) for restaurant in data]
           if data_objs is None or not data:
               print(f'No data for load')
               break
           # Обрабатываем полученные данные
           print(data_objs)
           offset += restaurants_data.limit
